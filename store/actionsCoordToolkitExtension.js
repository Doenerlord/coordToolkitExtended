...
import {convertSexagesimalFromString, convertSexagesimalToDecimal, convertSexagesimalFromDecimal} from "../../../src/shared/js/utils/convertSexagesimalCoordinates";
import {requestGfi} from "../../../src/shared/js/api/wmsGetFeatureInfo";
import layerFactory from "../../../src/core/layers/js/layerFactory";

/**
 * The actions for the CoordToolkit.
 * @module modules/coordToolkitExtension/store/actionsCoordToolkitExtension
 */
export default {
    /**
        ...
        if (rawLayer) {
            ...
            if (layerConfig) {
                const layer = layerFactory.createLayer(layerConfig),
                    layerExists = state.heightLayers.find(stateLayer => stateLayer.id === layer.get("id"));

                if (!layerExists) {
                    console.warn("CoordToolkit: the layer with id " + state.heightLayerId + " is not available in state.heightLayers. Adding it now.");
                    commit("setHeightLayers", [{
                        id: layer.get("id"),
                        name: layer.get("name"),
                        heightLayerInfo: state.heightLayerInfo,
                        heightElementName: state.heightElementName,
                        heightValueBuilding: state.heightValueBuilding,
                        heightValueWater: state.heightValueWater

                    }, ...state.heightLayers]);
                }
                commit("setHeightLayer", layer);
                commit("setHeightElementName", layerExists.heightElementName);
                commit("setHeightValueBuilding", layerExists.heightValueBuilding);
                commit("setHeightValueWater", layerExists.heightValueWater);
            }
        }
        if (!layerConfig) {
    ...
    /**
    * Creates a new WMSLayer to get the height from with id selected in dropdown from state.heightLayers and sets the layer to state.
    * @param {Object} context the vuex context
    * @param {Object} context.commit the commit
    * @param {Object} context.state the state
    * @param {Object} context.rootGetters the rootGetters
    * @param {String} layerId the id of the layer to get the height from
    * @returns {void}
    */
    changeHeightLayer ({commit, state, rootGetters}, layerId) {
        ...
        if (rawLayer) {
            ...
            if (layerConfig) {
                ...
                // get other heightlayer informations from heightlayers using layerId
                if (state.heightLayers) {
                    const heightLayer = state.heightLayers.find(layer => layer.id === layerId);

                    if (heightLayer) {
                        commit("setHeightElementName", heightLayer.heightElementName);
                        commit("setHeightValueBuilding", heightLayer.heightValueBuilding);
                        commit("setHeightValueWater", heightLayer.heightValueWater);
                    }
        ...
        if (!layerConfig) {
            console.warn("CoordToolkit: the layer with id " + layerId + " to retrieve height from is not available. Check the Id in config.json with path 'portalConfig.mainMenu.sections.coordToolkit.heightLayerId'!");
        }
    },
