...
import mutations from "../store/mutationsCoordToolkitExtension";
 ...
 * @module modules/coordToolkitExtension/components/CoordToolkitExtension
 * @vue-data {Object} eventHandler - The Cesium Event Handler.
 */
export default {
    name: "CoordToolkitExtension",
    components: {
        NavTab,
        InputText,
        FlatButton
    ...
    computed: {
        ...mapGetters("Modules/CoordToolkitExtension", [
            "active",
            "coordinatesEasting",
        ...
        selectedHeightLayerInfo () {
            if (!this.heightLayerId || !this.heightLayers) {
                return null;
            }
            const found = this.heightLayers.find(layer => layer.id === this.heightLayerId);

            return found ? found.heightLayerInfo : null;
        },
        /**
        ...
        formattedHeight () {
            ...
            return !isNaN(heightAsFloat)
                ? heightAsFloat.toFixed(2)
                : this.$t(this.height);
        }
    },
    watch: {
    ...
    unmounted () {
        ...
        this.unregisterListener({
            type: "click",
            listener: "Modules/CoordToolkitExtension/positionClicked",
    ...
    mounted () {
         ...
         */
        this.$nextTick(() => {
            if ((this.heightLayers && this.heightLayers.length > 0) || this.heightLayerId !== null) {
                if (this.heightLayerId === null) {
                    this.setHeightLayerId(this.heightLayers[0].id);
                    console.log("No heightLayerId set, setting to first available height layer.");
                }
    ...
    methods: {
        ...mapMutations("Modules/CoordToolkitExtension", Object.keys(mutations)),
        ...mapActions("Modules/CoordToolkitExtension", [
            "checkPosition",
            "changedPosition",
        ...
        heightLayerSelectionChanged (event) {
            if (event.target.value) {
                this.setHeightLayerId(event.target.value);
                this.changeHeightLayer(event.target.value);
            }
        },
        /**
         ...
         */
        isHeightLayerInfo () {
            return typeof this.heightLayerInfo === "string" || typeof this.selectedHeightLayerInfo === "string";
        },
        /**
        ...
        /**
         * Returns true, if current height layer is selected.
         * @param {Object} heightLayer the heightLayer
         * @returns {Boolean|null} true, if current height layer is selected, else null
         */
        isCurrentHeightLayerSelected (heightLayer) {
            if (heightLayer.id === this.heightLayerId) {
                return true;
            }
            return null;
        }
...
<template lang="html">
    <div id="coord-toolkit">
        ...
        >
            ...
            <div
                v-if="heightLayers && heightLayers.length > 0"
                class="form-floating mb-3"
            >
                <select
                    id="heightLayerSelect"
                    ref="heightLayerSelect"
                    class="form-select"
                    @change="heightLayerSelectionChanged($event)"
                >
                    <option
                        v-for="(heightLayer, i) in heightLayers"
                        :key="i"
                        :value="heightLayer.id"
                        :selected="isCurrentHeightLayerSelected(heightLayer)"
                    >
                        {{ heightLayer.name }}
                    </option>
                </select>
                <label for="heightLayerSelect">{{ "Höhenlayer" }}</label>
            </div>
            <div v-if="isEnabled('supply') && (heightLayer !== null || mapMode === '3D')">
                <InputText
                    :id="'coordinatesHeightLabel'"
                    ref="coordinatesHeightLabel"
                    :label="$t('additional:modules.coordToolkitExtension.heightLabelWienerNull')"
                    :value="$t(height)"
                    :readonly="true"
                    :placeholder="$t('additional:modules.coordToolkitExtension.heightLabelWienerNull')"
                    >
                    <div
                        v-if="isEnabled('supply') && !isMobile && showCopyButtons"
                        class="copyBtn"
                    >
                        <button
                            ...
                            type="button"
                            class="btn btn-outline-default inside px-3"
                            :title="$t(`additional:modules.coordToolkitExtension.copyHeightBtn`)"
                            @click="copyCoords(['coordinatesHeightLabel'])"
                        >
                            <span
                                class="bootstrap-icon"
                                aria-hidden="true"
            ...
            <div v-if="isEnabled('supply') && (heightLayer !== null || mapMode === '3D')">
                <InputText
                    ...
                    :label="$t('additional:modules.coordToolkitExtension.heightLabelAdria')"
                    :value="formattedHeight"
                    :readonly="true"
                    :placeholder="$t('additional:modules.coordToolkitExtension.heightLabelAdria')"
                >
                <div
                        v-if="isEnabled('supply') && !isMobile && showCopyButtons"
                        class="copyBtn"
                    >
                        <button
                            id="copyHeightAdriaBtn"
                            type="button"
                            class="btn btn-outline-default inside px-3"
                            :title="$t(`additional:modules.coordToolkitExtension.copyHeightBtn`)"
                            @click="copyCoords(['coordinatesHeightLabelAdria'])"
                        >
                            <span
                                class="bootstrap-icon"
                                aria-hidden="true"
                    ...
                    <div class="accordion-item">
                        ...
                        >
                            <div class="accordion-body">
                                <span v-if="isEnabled('supply') && (heightLayer !== null || mapMode === '3D') && selectedHeightLayerInfo">
