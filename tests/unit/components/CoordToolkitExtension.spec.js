...
import CoordToolkit from "@modules/coordToolkit/store/indexCoordToolkitExtension";
import actions from "@modules/coordToolkit/store/actionsCoordToolkitExtension";
...
describe("src/modules/coordToolkit/components/CoordToolkit.vue", () => {
    ...
    describe("CoordToolkit.vue watcher", () => {
        ...
        it("heightLayerSelectionChanged sets heightLayerId and calls changeHeightLayer", async () => {
            const value = "123",
                event = {
                    target: {
                        value: value
                ...
                changeHeightLayerSpy = sinon.spy(CoordToolkitComponent.methods, "changeHeightLayer");
            ...
            wrapper.vm.heightLayerSelectionChanged(event);
            await wrapper.vm.$nextTick();

            expect(store.state.Modules.CoordToolkit.heightLayerId).to.be.equals(value);
            expect(changeHeightLayerSpy.calledOnce).to.be.true;

            changeHeightLayerSpy.restore();
        });
    });
    describe("selectedHeightLayerInfo", () => {
        it("should return null if heightLayerId is not set", () => {

            store.commit("Modules/CoordToolkit/setHeightLayerId", null);
            wrapper = shallowMount(CoordToolkitComponent, {
                global: {
                    plugins: [store]
                }
            });
            expect(wrapper.vm.selectedHeightLayerInfo).to.be.null;
        });

        it("should return null if heightLayers is empty", () => {

            store.commit("Modules/CoordToolkit/setHeightLayerId", "123");
            store.commit("Modules/CoordToolkit/setHeightLayers", []);
            wrapper = shallowMount(CoordToolkitComponent, {
                global: {
                    plugins: [store]
                }
        ...
        it("should return null if no matching layer is found", () => {

            store.commit("Modules/CoordToolkit/setHeightLayerId", "456");
            store.commit("Modules/CoordToolkit/setHeightLayers", [{id: "123", heightLayerInfo: "Info 1"}]);
            wrapper = shallowMount(CoordToolkitComponent, {
                global: {
                    plugins: [store]
                }
        ...
        it("should return the info of the matching layer", () => {
            const layers = [
                {id: "123", heightLayerInfo: "Info 1"},
                {id: "456", heightLayerInfo: "Info 2"}
            ];

            store.commit("Modules/CoordToolkit/setHeightLayerId", "456");
            store.commit("Modules/CoordToolkit/setHeightLayers", layers);
            wrapper = shallowMount(CoordToolkitComponent, {
                global: {
                    plugins: [store]
                }
            });
            expect(wrapper.vm.selectedHeightLayerInfo).to.deep.equal("Info 2");
        });
    });

    describe("isCurrentHeightLayerSelected", () => {
        it("should return true if the layer's id matches the heightLayerId in store", () => {
            const heightLayer = {id: "123"};
            ...
            expect(wrapper.vm.isCurrentHeightLayerSelected(heightLayer)).to.be.true;
        });

        it("should return null if the layer's id does not match the heightLayerId in store", () => {
            const heightLayer = {id: "456"};
            ...
            expect(wrapper.vm.isCurrentHeightLayerSelected(heightLayer)).to.be.null;
        });

        it("should return null if heightLayerId in store is null", () => {
            const heightLayer = {id: "123"};
        ...
        it("should return null if the passed layer has no id", () => {
            const heightLayer = {name: "some layer"};
